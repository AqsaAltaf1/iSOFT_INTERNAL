...
class EvaluationApprovalNotification < Noticed::Base
  ...
  def message
    if params[:user].present?
      "<strong>#{params[:admin]&.full_name}</strong> has successfully Approved Your <strong>#{params[:evaluation].title}</strong> Evaluation".html_safe
    end
  end
